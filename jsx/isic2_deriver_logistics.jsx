...
// ============================================================
// EMPLOYER_CATEGORY -> ISIC Rev.4 TWO-DIGIT DIVISION names  [LOGISTICS]
// ============================================================
// Input 1: logistics_final_final.csv  (already has Employer_ISIC_4 from the
//          previous classifier)
...
// Output: logistics_final_isic2.csv
// ============================================================

export default function Isic2DeriverLogistics() {
    ...
    Papa.parse(file, {
      ...
      complete: (res) => {
        if (!res.meta.fields || !res.meta.fields.includes("Employer_ISIC_4")) {
          setStatus("This file has no Employer_ISIC_4 column — upload the output of the 4-digit classifier (logistics_final_final.csv).");
          return;
        }
        setRows(res.data);
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-6 font-sans">
      <div className="max-w-3xl mx-auto space-y-5">
        <h1 className="text-2xl font-bold text-teal-300">Employer → ISIC-2 Division Names — Logistics</h1>
        ...
        <div className="grid grid-cols-2 gap-4">
          <label className="block bg-slate-800 rounded-xl p-4 cursor-pointer hover:bg-slate-700">
            ...
            <p className="text-xs text-slate-400 mt-1">logistics_final_final.csv (has Employer_ISIC_4)</p>
        ...
        <div className="flex gap-3">
          ...
          {dlUrl && (
            <a href={dlUrl} download="logistics_final_isic2.csv"
               className="px-5 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-400 font-semibold inline-block">
              ⬇ Save logistics_final_isic2.csv ({dlRows.toLocaleString()} rows)
            </a>
          )}
        </div>
